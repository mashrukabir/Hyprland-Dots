...
wallpaper=$HOME/Pictures/wallpapers/drone.png
Waybar_Style="$HOME/.config/waybar/style/[Transparent] Crystal Clear.css"
...
while true; do
    ...
    case $confirm in
        ...
        [nN])
            ...
            echo "$(tput bold)$(tput setaf 7)If you are not sure, keep the us layout. You can change it later on! $(tput sgr0)"
...
for DIR in btop hypr kitty qt5ct rofi swaync swaylock wal waybar wlogout; do 
  DIRPATH=~/.config/"$DIR"
  if [ -d "$DIRPATH" ]; then 
...
# Wallpapers
echo "$(tput setaf 6) By default only a few wallpapers are copied..You can add your favorite wallapers in the Pictures/wallpapers folder.$(tput sgr0)"
printf "\n%.0s" {1..2}
